const Schema = require("../schema/index");
const Utils = require("../utils/utils");
const CONST = require("../config/constants");
const mongoose = require("mongoose");

const login = async (data) => {
  return await Schema.User.findOne({
    user: { $eq: data.user },
    password: { $eq: data.password },
  });
};

const create = async (data) => {
  return await Schema.User.create(data)
    .then((result) => {
      return result;
    })
    .catch((error) => {
      console.error("Error not saved:", error);
      return false;
    });
};

const gets = async (data) => {
  const result = await Schema.User.aggregate([
    {
      $match: {
        $and: [
          Utils.isEqual("user", data.user),
          Utils.isEqual("mail", data.mail),
          /* Utils.isGreatEqualDate(
            "createdAt",
            data.createdAt?.replace(/-/g, "")
          ),
          Utils.isLessEqualDate("createdAt", data.createdAt?.replace(/-/g, "")),
          { status: CONST.STATUS_CONSTANT.ACTIVE }, */
        ],
      },
    },
    {
      $sort: {
        date: -1,
      },
    },
  ]);
  console.log("gets", result);
  return result;
};

const update = async (data) => {
  return await Schema.User.findOneAndUpdate({ _id: { $eq: data._id } }, data)
    .then((result) => {
      console.log("Updated:", result);
      return result;
    })
    .catch((error) => {
      console.error("Error not Updated:", error);
      return false;
    });
};

const remove = async (data) => {
  return await Schema.User.findOneAndUpdate(
    { _id: { $eq: data._id } },
    { status: CONST.STATUS_CONSTANT.DELETE, deletedAt: new Date() }
  )
    .then((result) => {
      console.log("SoftDelete:", result);
      return result;
    })
    .catch((error) => {
      console.error("Error not SoftDelete:", error);
      return false;
    });
};

module.exports = {
  login,
  create,
  gets,
  update,
  remove,
};
